#include "opencv_toolkits/centroid_tracking.h"
#include <iterator>
...
std::vector<std::pair<int, std::pair<int, int>>> CentroidTracker::update(vector<vector<int>> boxes) {
    ...
    if (this->objects.empty()) {
        ...
        for (auto object : this->objects) {
            objectIDs.push_back(object.first); // id
            objectCentroids.push_back(make_pair(object.second.first, object.second.second)); // x, y
        }

//        Calculate Distances
        vector<vector<float>> Distances;
        for (int i = 0; i < objectCentroids.size(); ++i) {
            vector<float> temp_D; // dist from all other to i 
        ...
        // load rows and cols
        vector<int> cols; // the dist which nearest to object i
        vector<int> rows; 

        //find indices for cols
        for (auto v: Distances) {
            auto temp = findMin(v); // index of the nearest one in each distances<vector>
            cols.push_back(temp); // The index of the closest distance to Distances[i]
        }

        //rows calculation
        //sort each mat row for rows calculation
        vector<vector<float>> D_copy; // 按照距離長短排續的 distances
        for (auto v: Distances) {
            sort(v.begin(), v.end());
            D_copy.push_back(v);
        }
